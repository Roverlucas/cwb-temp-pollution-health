...
Climate--health policy in the Southern Hemisphere has intensified the
frequency of compound environmental extremes; however, the interactive
health impacts of cold spells concurrent with particulate matter
(PM$_{2{\cdot}5}$) accumulation remains understudied in subtropical
cities. This evidence gap leaves millions without targeted
early-warning systems for cold spells and polluted days.

\textbf{Methods}\quad
We analysed daily respiratory hospital admissions (DATASUS SIH/AIH,
ICD-10 J00--J99, 2022--2024; $N$=42\,716), temperature (INMET,
1961--2024) and air quality (fine particulate matter concentration ---
PM$_{2{\cdot}5}$) in Curitiba, Brazil ($\sim$1${\cdot}$8~million
inhabitants) using a methodological triangulation design. K-means
clustering first identified environmental regimes and their lagged
associations with hospitalisations without parametric assumptions;
distributed lag non-linear models (DLNM) with quasi-Poisson regression
then formally quantified the exposure--response relationship and lag
structure; and gradient-boosted trees (XGBoost) with SHAP feature
importance provided predictive convergence evidence. The MMT was
identified by grid search. Sensitivity analyses varied lag windows,
seasonal degrees of freedom, and excluded 2022 for residual COVID-19
effects.
...
Clustering identified a ``Clean Cold'' regime whose delayed
correlations with hospitalisations peaked at lags 3--10~days without
imposing any parametric lag structure. The DLNM formally confirmed
this pattern: MMT=17${\cdot}$8\,°C; cold extremes at the 1st
...
In contrast, heat effects were non-significant (P90 RR=1${\cdot}$06
[0${\cdot}$98--1${\cdot}$14]). Results were robust across all
sensitivity analyses ($\hat{\phi}$=1${\cdot}$32, Ljung--Box
$p$=0${\cdot}$14). SHAP analysis from the XGBoost model independently
ranked $T_\mathrm{min}$ as the dominant environmental predictor,
completing the three-method convergence. Monte Carlo projections
under CMIP6 SSP5-8${\cdot}$5 for 2050 showed that compound cold wave
and wildfire PM$_{2{\cdot}5}$ episodes could increase weekly
hospitalisations by up to 17\% and 55\%, respectively.
...
independent approaches --- structural-exploratory (clustering),
inferential (DLNM), and predictive (ML/SHAP) ---
strengthens this conclusion, and climate projections indicate that
compound extreme events will amplify this burden under future warming
scenarios. Current early-warning systems in subtropical South America
focus almost exclusively on heat alerts; integrating cold-spell and
air quality forecasts could reduce a substantial and preventable
respiratory burden.
...
%% ── Keywords ────────────────────────────────────────────────────────
\begin{keyword}
temperature extremes \sep cold spells \sep distributed lag non-linear model \sep
methodological triangulation \sep air quality \sep
respiratory morbidity \sep subtropical \sep fine particulate matter
\end{keyword}
...
To our knowledge, this is the first study to combine clustering, DLNM,
and machine learning analyses of temperature extremes, air pollution,
and respiratory hospitalisations in a subtropical South American city.
We show that cold extremes at the 1st percentile of
...
clustering first identified the delayed cold--hospitalisation pattern
without parametric assumptions, guiding the subsequent DLNM analysis;
SHAP-based feature importance from a gradient-boosted tree model then
independently ranked $T_\mathrm{min}$ as the dominant environmental
predictor. This
...
heat alerts. The 3--10-day delay between cold exposure and
hospitalisation peak provides a concrete window for preventive action
--- including targeted outreach to elderly and respiratory-vulnerable
populations, temporary shelter activation, and primary-care surge
planning. Exploratory analyses consistently identified PM$_{2{\cdot}5}$
...
induces systemic inflammation and oxidative stress, leading to
endothelial dysfunction and autonomic
imbalance.\cite{Brook2010} The synchronisation of these stressors
creates a ``double hit'' effect on the respiratory system, where
...
often results in non-linear impacts, where the aggregate consequence
exceeds the simple sum of individual components --- a phenomenon
...
rapidly growing metropolises in the Global South located in climatic
transition zones. In these regions, a significant ``adaptation gap''
persists: unlike high-latitude cities designed for low temperatures,
local housing infrastructure frequently lacks adequate thermal
insulation, rendering populations paradoxically more vulnerable to
even moderate cold spells. This creates a ``hidden vulnerability''
scenario where the health impact of temperature drops is
disproportionately amplified by co-occurring air pollution, a dynamic
often overlooked in Northern Hemisphere-centric
...
\paragraph{Climate projections and extreme-event simulations}
...
forecasts for 2030, 2050, and 2099. Monte Carlo simulations of
...
Models~B and~C). However, these null inferential results should be
interpreted in the context of low ambient concentrations
(median~4${\cdot}$1~µg/m$^3$) and 36${\cdot}$4\% sensor data
missingness, which likely limited statistical power (Supplementary
Methods~S.1).
...
\paragraph{Climate projections and extreme-event scenarios}
...
we projected daily respiratory hospitalisations for 2050
(eFigure~11). Under baseline 2050 climate conditions (without
extreme events), both models projected annual hospitalisation totals
similar to 2024 ($\sim$12\,300 admissions/year), reflecting the
...
vulnerability windows strengthens the projection robustness. These
forward-looking projections, while inherently uncertain, demonstrate
the operational potential of the ML models for climate-adaptive
early-warning systems and quantify the additional hospitalisation
burden attributable to compound extreme events.
...
and that PM$_{2{\cdot}5}$ operates as a structural co-factor
...
non-significant across all upper percentiles, besides extremely hot
days being predominant in the studied area (20${\cdot}$1\% against
...
morbidity signal. Whether this pattern generalises to other
subtropical cities requires multi-site replication, but the
...
thermal insulation and central heating, rendering populations
paradoxically more vulnerable to even moderate cold
spells.\cite{Gasparrini2015,Cohen2017} This creates a ``hidden
vulnerability'' scenario where the health impact of temperature drops
is disproportionately amplified by co-occurring air pollution --- a
dynamic often overlooked in Northern Hemisphere-centric literature
that assumes adequate indoor climate
...
conclusion --- that cold stress is the primary environmental driver
of respiratory hospitalisations, with delayed effects, and that
