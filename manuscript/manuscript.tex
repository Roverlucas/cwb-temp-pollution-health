...
\title{The compounding respiratory hospitalisation risk of cold spells
       and particulate matter in a subtropical metropolis: a time-series analysis}
...
\author[utfpr]{Pedro Augusto Breda Font\~{a}o}
\ead{pedrofontao@ufpr.br}
\author[utfpr]{Leonardo Jos\'{e} Rossoni Quadros}
\author[gtao]{Eduardo Tadeu Bacalhau}
\ead{bacalhau@ufpr.br}
\author[ufpr]{Camila Carpenedo}
\author[utfpr]{Yara de Souza Tadano}
\author[ufpr]{Ricardo Henrique Moreton Godoi}
...
\address[gtao]{Research Group of Technology Applied to Optimization (GTAO), Federal University of Paran\'{a} (UFPR), Curitiba, PR, Brazil}
...
simulations under CMIP6 SSP5-8${\cdot}$5 for 2050 showed
...
simulations indicate that compound extreme events will
...
Flex-II sensor (Sensor ID~90875), daily resolution. Raw readings
...
XGBoost and MLP models were applied to statistically downscaled
temperature projections from the CMIP6 ensemble (EC-Earth3-Veg-LR
model, SSP5-8${\cdot}$5 high-emission pathway) for the grid cell
containing Curitiba ($\sim$100~km resolution), generating daily
respiratory hospitalisation scenarios for 2030, 2050, and 2099. Monte Carlo simulations of
...
suggests that air pollution may operate as a secondary environmental
...
$\hat{\phi}$=1${\cdot}$32) identified an MMT of 17${\cdot}$8\,°C, closely aligned with the 75th percentile
...
\paragraph{Exploratory scenario and extreme-event simulations}
Applying the trained XGBoost and MLP models to CMIP6
SSP5-8${\cdot}$5 temperature projections (EC-Earth3-Veg-LR; see §2.6)
for Curitiba's grid cell,
we estimated daily respiratory hospitalisations for 2050
(eFigure~11). Under baseline 2050 climate conditions (without
extreme events), both models projected annual hospitalisation
...
vulnerability windows strengthens the projection robustness.
These forward-looking exploratory scenarios, while inherently
...
alone, and reduces the concern that the results are artefacts of a
...
Extension to other subtropical Latin American cities following
...
clustering (Polluted Heat cluster with the highest admissions), and SHAP
...
CC: Methodology (climate analysis), Validation, Writing --- review \& editing.
