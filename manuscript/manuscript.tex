...
completing the three-method convergence.
...
hospitalisations at prediction horizons of 0, 1, 5, and 7~days. The
7-day limit reflects the practical ceiling of operational weather
forecast accuracy and differs from the epidemiological lag window
(0--21~days) used in the DLNM to capture the full delayed attribution
of temperature effects: the ML horizon asks ``how far ahead can we
predict?'', whereas the DLNM lag asks ``how long does the biological
effect persist?'' SHAP values\cite{Lundberg2017} quantified feature
importance. Because gradient-boosted trees use recursive partitioning
rather than spline bases and optimise predictive accuracy rather than
estimating relative risks, agreement between SHAP rankings and DLNM
findings constitutes methodological convergence. Full details are reported in
...
XGBoost achieved the best overall forecasting performance at a
1-day prediction horizon (MAPE=18${\cdot}$0\%, RMSE=8${\cdot}$10;
eTable~8), with competitive performance maintained at lag~0
(MAPE=19${\cdot}$4\%) and lag~5 (MAPE=20${\cdot}$3\%). MLP was the
best-performing model at the 5-day horizon (MAPE=19${\cdot}$1\%).
The fact that 1-day-ahead predictions outperformed same-day
predictions is consistent with the delayed effect structure
identified by the DLNM: environmental conditions from the previous
day carry stronger signal for hospitalisation prediction than
concurrent-day values.
...
Applying the trained XGBoost model to CMIP6 SSP5-8${\cdot}$5
temperature projections (EC-Earth3-Veg-LR) for Curitiba's grid cell,
we projected daily respiratory hospitalisations for 2030, 2050, and
2099 (Supplementary Methods~S.3). Monte Carlo simulations of extreme
events --- cold waves ($-$2--5\,°C for 3--7~days) and wildfire
PM$_{2{\cdot}5}$ spikes (70--100~µg/m$^3$ for 7--14~days) ---
illustrate that the combination of cold extremes and pollution
episodes could produce substantially higher hospitalisation peaks
than either stressor alone. These forward-looking projections,
while inherently uncertain, demonstrate the operational potential of
the ML models for climate-adaptive early-warning systems.
